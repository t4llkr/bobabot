...
async def hen(ctx):
    ...
    #cum = await ctx.send(file = f, embed=embed)
...
async def dice(ctx, message):
    ...
    if "-" in message:
        ...
        await ctx.send('Выпавшее число на кости из ' + str(message) + ' граней:\n' + ':game_die: **' + str(diap) + '**')
    with open('log.txt', 'a') as file:
...
async def report(ctx, *args):
    ...
    if len(quote_text) > 56:
        await ctx.send('Репорт отправлен. Спасибо за помощь в разработке.')
        tofile = str(quote_text.replace('Теперь это задокументировано и сохранено. Вот цитата:\n> ', ''))
        with open('quotes.txt', 'a') as file:
            file.write(str(datetime.now()) + ' ' + str(author) + ' ' + tofile + '\n')
...
async def fruit(ctx, *args):
    ...
    if len(input) > 1 and str(input) == '#help':
        ...
        #site = 'https://delicious-fruit.com/ratings/game_details.php?id=11235'
        l = requests.get(site)
        soup = BeautifulSoup(l.text, 'lxml')
        ...
        se = str(soup.find("span", {"id": "avgRating"}))
        sed = str(se.replace('<span id="avgRating">', 'Качество: ').replace('</span>', '\n'))
        sed = sed.replace(' / ', '/')

        te = str(soup.find("span", {"id": "avgDifficulty"}))
        ted = str(te.replace('<span id="avgDifficulty">', 'Сложность: ').replace('</span>', '\n'))
        ted = ted.replace(' / ', '/')

        rew = str(soup.find('h2', text = re.compile('Reviews:')))
        if rew == 'None':
            rew = str(soup.find('h2', text = re.compile('Review:')))

        def multiple_replace(target_str, replace_values):
                for i, j in replace_values.items():
                    target_str = target_str.replace(i, j)
                return target_str
        rep = {'<h2 style="clear:both;">': '', "<h2>": "", "Reviews:</h2>": "оценок", "Review:</h2>": "оценка"}
        obsh = multiple_replace(rew, rep)
        if '3 ' in obsh or '2 ' in obsh or '4 ' in obsh:
            obsh = obsh.replace('оценок', 'оценки')
        if '1 ' in obsh:
            obsh = obsh.replace('оценок', 'оценка')
        if '11 ' in obsh or '12 ' in obsh or '13 ' in obsh or '14 ' in obsh:
            obsh = obsh.replace('оценки', 'оценок')
        await ctx.send(ee + '\n' + sed + ted + obsh + '\n' + de)   
        with open('log.txt', 'a') as file:
...
async def torus(ctx, *, arg):
    layout = dict(zip(map(ord, "qwertyuiop[]asdfghjkl;'zxcvbnm,./`"
                           ...
                           'ЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮ,Ё'))
    a = '{}'.format(arg)
    await ctx.send('```' + (a.translate(layout)) + '```')
...
async def vkpub(ctx):
    ...
    await ctx.send("https://vk.com/public" + str(id))
    with open('log.txt', 'a') as file:
...
async def vkpage(ctx):
    ...
    import random
    author = ctx.message.author
    id = random.randint(1, 670000000)
    await ctx.send("https://vk.com/id" + str(id))
    with open('log.txt', 'a') as file:
...
async def sound(ctx, *, arg):
    ...
    if os.path.isfile(path):
        ...
        if str(input) == "help":
            ...
            await ctx.send( '**Список всех звуков по алфавиту:** \n \n' + files[:-2] )
        else:
            await ctx.send('Долбоеб, такого звука нет')
